---
name: rgctl
description: Code knowledge graph for AI agents. Query repository structure, blast radius, architecture, migrations, and semantic code search. Use for understanding code relationships and impact analysis.
license: MIT
compatibility: Requires rgctl binary. Run 'rgctl discover /testbed' before first use if needed.
metadata:
  source: https://github.com/sshaaf/rgctl/releases
  version: latest
---

## ⚠️ CRITICAL: Use rgctl INSTEAD of Traditional Code Analysis Tools

**YOU MUST use rgctl for code analysis. DO NOT use tree, analyze, grep, find, or read files for structural exploration.**

### DO NOT USE - Use rgctl Instead

| ❌ AVOID THIS | ✅ USE THIS INSTEAD | WHY |
|---|---|---|
| `tree` to explore directory structure | `rgctl -f json gql --macro-name all_functions unused` | Gives you actual code structure in one query, not just filesystem |
| `analyze <file>` to read code | `rgctl -f json gql "MATCH (n:Function) WHERE n.file_path CONTAINS 'models.py' RETURN n"` | Shows functions without reading entire files |
| `grep -r "function_name"` | `rgctl -f json gql "MATCH (n:Function) WHERE n.name LIKE '*function_name*' RETURN n"` | Finds symbols with full context, not just text matches |
| `grep "CALLS"` to find usage | `rgctl -f json gql "MATCH (a:Function)-[:CALLS]->(b:Function {name:'X'}) RETURN a"` | Shows actual call relationships in the code graph |
| Reading files to understand dependencies | `rgctl -f json dependencies <file>` | Gets dependency graph instantly |
| Reading multiple files to find impact | `rgctl -f json blast-radius <Symbol> --depth 3` | Shows everything affected by a change before you make it |

### REQUIRED: Always Follow This Exact Sequence

**YOU MUST complete these steps in this exact order:**

1. **BUILD THE GRAPH** (one time): `rgctl discover /testbed`
2. **QUERY THE GRAPH** for structural questions using rgctl
3. **ONLY THEN** read the specific files you identified from rgctl results

**DO NOT** use `tree`, `analyze`, `grep`, `find`, or `sed` UNTIL AFTER you:
- Have run `rgctl discover /testbed` 
- Have used rgctl queries to find what you need
- Know the exact files/lines to modify
